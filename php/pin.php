...
<div class="group">
    ...
    <? if($value == 'not_exported'):?>
        <img id="pin_<?=$pin?>_value" class="gpio_value" src="/images/indicator/<?=$value?>.png" alt="" title="" />
    <? else:?>
        <img id="pin_<?=$pin?>_value" class="gpio_value" src="/images/indicator/<?=$value?>.png" alt="<?= $value ? 'on' : 'off'?>" title="<?= $value ? 'on' : 'off'?>" />
    <? endif?>
